...
imdb <- as.data.frame(readRDS("imdb2020.RDS"))
```
...
As explained in the video, if we don't store the results of our commands, there is no way to re-use them. This is actually true for the great majority of R command, not just those involving the core functions of dplyr. So to perform a sequence of manipulations, we need to either:
...
For example, if we wanted to perform summaries on the entries of type "movie" only, we could do the following:

```{r sequence_1, exercise = TRUE}
imdb_movies <- filter(imdb, type=="movie")
summarize(imdb_movies, entries=n(), meanVotes=mean(numVotes), meanRating=mean(averageRating), sdRating=sd(averageRating))
```
First we use `filter()` to keep only the entries of type "movie". We store the result as a dataframe object, arbitrarily called "imdb_movies", and then we use the `summarize()` command of the last workbook, but with the newly created object as the first argument instead of the full imdb dataset.



**Question: change the command to get summaries for the entries of type "tvSeries"**
...
summarize(imdb_tvSeries, entries=n(), meanVotes=mean(numVotes), meanRating=mean(averageRating), sdRating=sd(averageRating))
```
...
```
It looks like nothing happened to our dataset, but it's just because the grouping is invisible. We need to apply another function to see the effect of group_by(). Let's store our grouped dataframe in an object called say, "imdb_type", and use this object as the first argument of the function summarize()

```{r group_2, exercise = TRUE}
imdb_type <- group_by(imdb, type)
summarize(imdb_type, entries=n(), meanVotes=mean(numVotes), meanRating=mean(averageRating), sdRating=sd(averageRating))
```

**Question: Investigate the result of the previous command. What are the 3 video games in the imdb dataset? Are you surprised by such a low number of entries? Do you trust the values for the average number of votes? Double-check the `Appendix: "imdb" dataset` to remind yourself how the dataset was constructed.**
...
The effect of `group_by()` on the result of `summarize()` is very intuitive. We obtain the calculated summaries for each of the groups defined by group_by. At first, it might be slightly less obvious that `group_by()` is also very useful in combination with `filter()` or `mutate()`. But consider the case where we would like to obtain for each type of entry, the one that received the highest number of votes. Using `group_by()`, then `filter()` would give the intended result.
...
Our code starts to be quite messy though, with lots of intermediary dataframes that we are not really interested in. One thing you may suggest to simplify our set of commands is to have only one intermediary dataframe, that we overwrite. Something like
...
The command is there fore equivalent to `filter(imdb, numVotes==max(numVotes))`

Instead of placing the function filter to the right of the pipe, we can, and usually should place it below the pipe, with a little indentation. I's good practice for readability, and it doesn't change anything. R will see the pipe and look for for the next command. This command happens to be below the pipe rather than on its right. Note that the couple of spaces to create the indentation will be created automatically when you'll be using Rstudio.
...
What is great with pipes is that the *what's on my left* can well be a command itself, if the result of the command is a dataframe. So we can redo the last commands of the previous section, using pipes as follows.
...
We start with the dataset `imdb`. The pipe next to it will make it the first argument of the function `group_by()` that follows. The next pipe makes the resulting command the first argument of the first function `filter()`. The next pipe takes the result of all of this and make it the first argument of the next function, which is `ungoup()`. And the last pipe make the resulting dataframe, the first argument of the second function filter. Here we go. We have a neat and concise command! Note that when using pipes, the second argument of all the dplyr functions become the first argument, since the dataframe argument is taken care of by the pipe.
...
Let's now continue exploring this imdb dataset using all the knowledge acquired so far.


If you are a bit into science-fiction like me, you may like to know what are the most popular entries of this genre, for each type of entry.
...
```
So here we used filter twice. We first used it to keep only the entries of genre `sci-fi`, then we grouped the data by type and finally we used filter again to keep only the entry of each type that has received the highest number of votes.

Note that the order of our commands matter. Try and see if you can understand what happens when you change the order of these commands, as well as when you combine the two `filter()` together.


Also let's not hesitate to add a `select()` at the end of our command, to get rid of the columns that we don't want and rearrange the columns we want for better redability.
...
Of course, if we want to store the result of our command into a dataframe object for later use, we can do that. As said before, moving the lines below the pipe instead of to their right doesn't change anything for R.
...
Let's try and make a little scatter plot showing the average rating (on the y axis) of the most popular movie released in the year, where year is on the x axis.


```{r all_4, exercise = TRUE}
imdb %>%
  filter(type=="movie")%>%
  group_by(year) %>%
  filter(numVotes==max(numVotes)) %>%
...
Do you follow the logic? We filter our dataset to keep only the entries of type movie, then we group those movies by year. We then use filter again to only keep the most popular movie released in that year, and we finally make our scatter plot, where we don't need to indicate the usual "data=..." as the first argument, since the pipe already does it for us.

Looking at this graph, it looks like if we strictly focus on the best rated movie, there is an upward trend in quality. But what if we plot the average of the average ratings, instead of the average rating of the most popular movie?

```{r all_5, exercise = TRUE}
imdb %>%
  filter(type=="movie")%>%
  group_by(year) %>%
  summarize(meanRating=mean(averageRating)) %>%
  ggplot(aes(x=year, y=meanRating))+
    geom_point()

```

Indeed, the story is quite different. Let's be careful with our interpretations though, as the way our dataset and the imdb database are built, makes comparisons between years a bit tricky.



Finally, once you get familiar with the pipe operator and all the functions we've seen so far, it quickly becomes pretty easy to make more complicated sequences of manipulations. Take some time to analyse and understand the following command.

```{r all_6, exercise = TRUE}
imdb %>%
  group_by(director) %>% 
...
**Question: What does the command above do? Which line could we remove by modifying the inside of the function summarize()?**
...
```{r all_7-solution}
# 1- The command ranks the directors who have more than 10 entries in the imdb dataset and an average number of votes over all their entries greater than 10000, by decreasing average of all the rating of their entries.
# 2- the line with the function mutate() could be deleted if we were directly writing meanVotes=mean(numVotes) instead of sumVotes=sum(numVotes) in the summarize() function

imdb %>%
  group_by(director) %>% 
  summarize(n=n(), meanRating=mean(averageRating), meanVotes=mean(numVotes), medianLength=median(length)) %>%
  filter(n>10 & meanVotes>10000) %>%
  arrange(desc(meanRating)) %>% 
  select(director, n, meanRating, medianLength)
...
The package dplyr has a number of other useful functions that we have not explored, like slice(), rename(), count(), as well as convenient variations of the core functions that have learnt to use. See the section `Appendix: Useful reference links` for more information.
...
The official list of functions and documentation of the dplyr package: <a href="https://r4ds.had.co.nz/transform.html" target="_blank">https://dplyr.tidyverse.org/reference/index.html</a> 
